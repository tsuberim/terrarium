...
| ID | Priority | Debt | Impact | Direction |
|----|----------|------|--------|-----------|
| TD-CODE-1 | **P2** | `strategies/` → WAT sync is manual (`build-strategies.sh`); CI doesn't build | Drift if someone edits Rust without sync | **Done** — `scripts/check-strategies-sync.sh` in CI |
